#!/usr/bin/env python3
"""
DNS Client with 22-Point Timing Chain Tracking
Tracks timing through: Client → Proxy → RR → ANS → Response Path
Including encryption/decryption at each hop
"""
...
class DNSClientWithDetailedTiming:
        ...
        if "forward_path" in timing_data:
            for long_key, short_key in forward_map.items():
                if timing_data["forward_path"].get(long_key) is not None:
                    # Store as offset from start time in microseconds to save space
                    start_time = timing_data["forward_path"].get("T01_client_start", 0)
    ...
    def create_dns_query_with_timing(self, domain, query_type="A"):
        ...
        timing_data = {
            ...
            "forward_path": {
                ...
                "T02_proxy_recv": None,
                "T03_proxy_encrypt_start": None,
                "T04_proxy_encrypt_end": None,
                "T05_proxy_send_to_rr": None,
                "T06_rr_recv": None,
                "T07_rr_encrypt_start": None,
                "T08_rr_encrypt_end": None,
                "T09_rr_send_to_ans": None,
                "T10_ans_recv": None,
                "T11_ans_encrypt_start": None,  # Only if custom domain
                "T12_ans_encrypt_end": None,    # Only if custom domain
                "T13_ans_send_response": None
            },
            "response_path": {
                "T14_rr_recv_response": None,
                "T15_rr_decrypt_start": None,
                "T16_rr_decrypt_end": None,
                "T17_rr_send_to_proxy": None,
                "T18_proxy_recv_response": None,
                "T19_proxy_decrypt_start": None,
                "T20_proxy_decrypt_end": None,
                "T21_proxy_send_to_client": None,
                "T22_client_recv": None  # Will be set when response arrives
            }
        }
    ...
    def expand_compact_timing(self, compact_timing):
        ...
        try:
            ...
            # Get start time reference (assume first timestamp is client start)
            start_time_offset = None
            if "f" in compact_timing and "c1" in compact_timing["f"]:
                start_time_offset = compact_timing["f"]["c1"]
                # Convert back to absolute time (approximate)
                base_time = time.perf_counter() - (start_time_offset / 1000000.0)
            else:
            ...
            for short_key, offset_us in compact_timing.get("f", {}).items():
                if short_key in forward_map:
                    ...
                    # Convert microsecond offset back to absolute time
    ...
    def analyze_22_point_timing(self, timing_data):
        ...
        print(f"   📅 Started: {timing_data['metadata']['start_time_human']}")
        ...
        # Add forward path timestamps
        for key, value in timing_data['forward_path'].items():
            if value is not None:
                all_timestamps[key] = value
        
        # Add response path timestamps  
        for key, value in timing_data['response_path'].items():
            if value is not None:
                all_timestamps[key] = value
        ...
        try:
            ...
            print(f"   ⚠️  Some timing data missing: {e}")
...
def main():
    ...
    client = DNSClientWithDetailedTiming()
