...
ANS_SCRIPT="non_encrypt.py"  # Replace with your actual ANS script name
...
process_ans_data() {
    ...
    # Extract timestamps - Updated for no encryption logs
...
monitor_ans() {
    ...
    tail -n0 -F "$TEMP_LOG" | {
        while read -r line; do
            ...
            # Updated trigger for no encryption mode
...
cleanup() {
    ...
    if [ -f "$ANS_LOG_FILE" ]; then
        ...
        echo "=== ANS MONITORING SUMMARY (NO ENCRYPTION) ===" >> "$ANS_LOG_FILE"
...
echo "============================================" >> "$ANS_LOG_FILE"
echo "Timestamp,Domain,Run,TW12,TW13,TW14,TW15,TW16,TW17,TW18,ANS_RTT_ms,Status" >> "$ANS_LOG_FILE"
echo "" >> "$ANS_LOG_FILE"
