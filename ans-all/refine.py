...
class EncryptedANSServer:
    ...
    def load_keys(self):
        ...
        try:
            ...
            # Calculate max payload sizes with PKCS1v15 padding
            ans_key_size = self.ans_private_key.key_size // 8
            rr_key_size = self.rr_public_key.key_size // 8
            
            self.max_decrypt_size = ans_key_size - 11  # PKCS1v15 padding overhead
            self.max_encrypt_rr_size = rr_key_size - 11
            
            print(f"Max decrypt payload (ANS): {self.max_decrypt_size} bytes")
            print(f"Max encrypt payload (RR): {self.max_encrypt_rr_size} bytes")
    ...
    def decrypt_confidentiality_layer(self, encrypted_data):
        """Decrypt data using ANS's private key with PKCS1v15 - optimized version"""
        try:
            # Read the header to determine if chunked or single
            chunk_count = struct.unpack('!B', encrypted_data[:1])[0]
            encrypted_payload = encrypted_data[1:]
            
            if chunk_count == 1:
                print("✓ Single chunk decryption from RR")
                # Single chunk - decrypt directly
                decrypted_data = self.ans_private_key.decrypt(
                    encrypted_payload,
                    padding.PKCS1v15()
                )
                return decrypted_data
            else:
                print(f"⚠️  Multi-chunk decryption from RR ({chunk_count} chunks)")
                # Multiple chunks
                decrypted_chunks = []
                offset = 0
                ...
                for i in range(chunk_count):
                    # Read chunk length
                    chunk_len = struct.unpack('!H', encrypted_payload[offset:offset+2])[0]
                    offset += 2
                    
                    # Read and decrypt chunk
                    encrypted_chunk = encrypted_payload[offset:offset+chunk_len]
                    offset += chunk_len
    ...
    def decrypt_from_rr(self, encrypted_data):
        """Decrypt 2-layer encrypted packet from RR using optimized PKCS1v15"""
        try:
            print(f"🔓 Starting packet decryption from RR, data length: {len(encrypted_data)} bytes")
            
            # Layer 1: Decrypt confidentiality layer using optimized PKCS1v15 decryption with ANS private key
            print("🔓 Layer 1: Decrypting confidentiality layer (PKCS1v15)...")
            signed_data = self.decrypt_confidentiality_layer(encrypted_data)
    ...
    def encrypt_confidentiality_layer_for_rr(self, data):
        """Encrypt data for RR using PKCS1v15 padding - optimized version"""
        try:
            data_len = len(data)
            print(f"📊 Data to encrypt for RR: {data_len} bytes")
            print(f"📊 Max single chunk size: {self.max_encrypt_rr_size} bytes")
            
            # Try to avoid chunking for typical DNS responses
            if data_len <= self.max_encrypt_rr_size:
                print("✓ Single chunk encryption to RR (no chunking needed)")
                encrypted_data = self.rr_public_key.encrypt(
                    data,
                    padding.PKCS1v15()
                )
                # Return with a header indicating single chunk
                return struct.pack('!B', 1) + encrypted_data
            else:
                print(f"⚠️  Data too large for RR, chunking required ({data_len} > {self.max_encrypt_rr_size})")
                # Fallback to chunking if absolutely necessary
                encrypted_chunks = []
                chunk_count = 0
                
                for i in range(0, data_len, self.max_encrypt_rr_size):
                    chunk = data[i:i + self.max_encrypt_rr_size]
                    encrypted_chunk = self.rr_public_key.encrypt(
                        chunk,
                        padding.PKCS1v15()
                    )
                    encrypted_chunks.append(struct.pack('!H', len(encrypted_chunk)) + encrypted_chunk)
                    chunk_count += 1
                
                print(f"📊 Created {chunk_count} chunks for RR")
                # Return with header indicating chunk count
                return struct.pack('!B', chunk_count) + b''.join(encrypted_chunks)

        except Exception as e:
            print(f"❌ Confidentiality encryption error for RR: {e}")
            return None
    
    def encrypt_for_rr(self, dns_response):
        """Encrypt response for RR using 2-layer asymmetric encryption with optimized PKCS1v15"""
        try:
            ...
            # Layer 2: Encrypt with RR public key (confidentiality) - optimized PKCS1v15
            print("🔒 Layer 2: Encrypting with RR public key...")
            encrypted_response = self.encrypt_confidentiality_layer_for_rr(signed_response)
            
            if encrypted_response is None:
                print("❌ Failed to encrypt for RR")
                return None
    ...
    def start_server(self):
        ...
        try:
            ...
            print("=== Optimized 2-Layer Asymmetric Encrypted ANS Server ===")
            print("Layer 1: Authentication (RSA Digital Signatures)")
            print("Layer 2: Confidentiality (RSA PKCS1v15 - Optimized)")
            print("Optimizations: No JSON/Base64, PKCS1v15 padding, Minimal chunking")
            print("==========================================================")
